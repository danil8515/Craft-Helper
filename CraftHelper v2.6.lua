...
function imgui.OnDrawFrame()
	...
	if main_window.v then
		...
		if imgui.BeginMenuBar() then
			if imgui.BeginMenu(u8'Настройки') then
				 ...
				 if imgui.MenuItem(u8'Информация') then
				 	sampAddChatMessage('[Craft Helper] {FFFFFF}Автор: Kopnev Scripts.', 0xF1CB09)
				 	sampAddChatMessage('[Craft Helper] {FFFFFF}Вк: vk.com/kscripts', 0xF1CB09)
				 end
				imgui.EndMenu()
			end
			imgui.EndMenuBar()
		...
		for i, v in ipairs(craft) do
			if imgui.Button(u8(v['typee']), imgui.ImVec2(95, 25)) then
				mq()
				vkladki[1] = true
				ss = i 
				sel = -1 
		...
		if sel ~= -1 then
				...
				imgui.Checkbox(u8"Premium VIP ( x2 шанс )", pVIP)
				imgui.Separator()
				imgui.Text(u8'Требования:')
					...
					imgui.Text(u8'  Шанс успеха:') imgui.SameLine(125) 
					if pVIP.v then 
						if sel1['chance'] ~= "100" then 
							local chance = sel1['chance'] * 2	
							imgui.Text(chance.."%") 
						else 
							imgui.Text(sel1['chance'].."%") 
						end
					else 
						imgui.Text(sel1['chance'].."%") 
					end
